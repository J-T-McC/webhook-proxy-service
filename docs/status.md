...
| 4 | Queued processing (FIFO & Async) | Technical Design | Principal Engineer (plan-04) | Depends on #1 (Done). V3 resolved (ADR-005: Redis transport, beyond-Redis seam open/not built); V8 RESOLVED—Owner deferred, no perf targets (2026-08-04). Open at design time: Q-04-01 (queue mechanism, processing_mode data-model gate) + mid-flight-mode-change UI semantics | PRD-04 Approved (Owner 2026-08-04); design-04 Approved (PM 2026-08-04): processing-mode Select on create/edit form, Show-page Badge, Index Processing column |
