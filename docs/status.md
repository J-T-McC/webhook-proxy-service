...
| 8 | Payload mapping / reshaping | **On hold indefinitely (Owner, 2026-08-29)** | — | Not needed for MVP and not scheduled. Artifacts parked, not withdrawn; zero implementation exists, so the hold unwinds nothing in code. **Anything naming #8 as a dependency should assume it is not coming rather than wait for it.** See § Item #8 — carried forward | PRD-08 Approved (Owner, 34 ACs); design-08 PM-approved; plan-08 self-certified except its two Owner gates, deliberately not approved; ADR-019 Proposed |
| 9 | Multi-format ingestion | **Cancelled (Owner, 2026-08-29)** | — | Ingest-and-forward already works and has since #1: `content_type` is recorded, the body is stored as received, `Content-Type` is forwarded rather than stripped, and delivery sends the exact bytes back out unmapped. Parsing non-JSON into a canonical JSON representation was only ever for #8 and #12, both now on hold, so it has no consumer. **Live consequence: sensitive-field obfuscation is JSON-only and non-JSON payloads bypass it by design (PRD-10 AC22)** | — |
...
| 11 | Analytics / stats | Done | — | **The independent Review gate was never run** — the Owner merged on the implementation's own self-verification, so no `docs/reviews/review-11-*.md` exists. Recorded gap, not outstanding work. **V8 renewed as a deferral, still open against #4 and #11.** See § Item #11 — live detail | PR #17 (`d9fed9c`). No ADR — the two gates are the decision record |
| 12 | Change detection | **On hold indefinitely (Owner, 2026-08-29)** | — | Depends on #8, itself on hold. Not scheduled. The narrower read still stands if it is ever revived: #12 needs only the expected-incoming-structure slice, not the mapping editor | — |
...
| 18 | Destination validation | Done | — | None. Re-review **Approve with follow-ups**, deep tier, 2026-08-31 — all ten original findings closed, T19–T20 accepted. Its three new Minors are closed in `d9ecf87`. **The live browser pass has never been run** and is the one verification this feature was promised and has not received; the re-review section lists the six things it must cover. ADR-027 Accepted (Project Owner, 2026-08-31); Q-18-01 Answered; Q-18-02 closed | `docs/tasks/destination-validation/` — 18 tasks across 5 milestones, split per `docs/tasks/README.md`. **All milestones shipped 2026-08-31** (M5 last); completion notes per task. M5 reopened for T19–T20, added after the first review pass to cover AC35 — the Project Owner approved the schema change they need on 2026-08-31 and chose to implement AC35 rather than defer it. PR #55 (`4ef662a`), merged 2026-09-01. plan-18 Approved; PRD-18 Approved (45 ACs); `design-18` reference only, Designer gate dropped by Owner ruling | PRD-18 Approved (Project Owner, 2026-08-31; 45 ACs). `docs/plans/plan-18-destination-validation.md` self-certified except ADR-027's decisions. Q-18-01 Answered — pause's enforcement points are the wrong granularity; the gate is four points, not two. `design-18` written as reference; **the Designer gate was dropped by Owner ruling, 2026-08-31**. **Narrows #1's fan-out contract** to validated destinations |
| 19 | Load testing harness | Done | — | None. Local-only by design; not wired into CI. All five scenarios run and baselined. `fifo-head-of-line` demonstrates correctness under a slow destination (all deliveries arrive, ordering holds) but does **not** quantify isolation — it records no per-proxy timing, so "a slow destination stalls only its own line" is not yet measured | `docs/briefs/load-testing-harness.md` — brief lane, no PRD, design, plan or review. Harness in `tests/load/`, results in `tests/load/history.ndjson`. Merged 2026-09-01, PR #66 (`922667e`). Baselines recorded for all five scenarios: 25.0-33.1 database reads per event, and 20 FIFO proxies advancing in parallel with zero ordering violations |
