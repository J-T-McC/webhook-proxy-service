...
| ADR-021 secret handling & rotation (`proxy_secrets` rows for rotating secrets, columns for the credential, recoverable encryption throughout) | **Proposed** (plan-10 flag 2) | — |
| ADR-022 inbound verification at the ingest boundary (the seam, and the closed two-case scheme registry) | **Proposed** (plan-10 flag 3) | — |
...
| ADR-024 field obfuscation & revealed-payload envelope (adopts an alternative ADR-017 rejected by name) | **Proposed** (plan-10 flag 4) | — |
...
| 10 | Sensitive data handling | **Technical Design** | Principal Engineer → **Project Owner (4 gates)** + **Product Manager (Q-10-04)** | **plan-10 written; four Owner gates open and none is self-certified.** **Flag 1 — data model:** one new `proxy_secrets` table plus six columns (three on `proxies`, three on `destinations`), enumerated exactly in plan-10 § Data Model. Its only index is a constraint, `UNIQUE(proxy_id, purpose, is_current)`. Rollback is destructive to every stored secret. **Coupled to flag 2** — a `no` there substitutes the fixed-column change set. **Flag 2 — ADR-021** (Proposed): rotating secrets as rows with an expiry, the non-rotating destination credential as columns, recoverable encryption throughout, never hashed. Written to the **Project Owner's ruling of 2026-08-27** asking for a table and relation rather than a fixed column pair. The rejected fixed-column alternative is enumerated in full so the gate is a choice, not a rubber stamp. **Flag 3 — ADR-022** (Proposed): the verification *seam* — inside `IngestController` after proxy resolution and before capture — and the closed two-case registry AC50's future decisions are taken against. **Flag 4 — ADR-024** (Proposed): the payload endpoint's response becomes a JSON envelope for JSON payloads only, adopting an alternative **ADR-017 rejected by name**. **ADR-023 is Accepted by ratification, deliberately not a flag** — PRD-10's approval already ratified it. **A second Owner ruling of 2026-08-27 contradicts approved documents**: outbound signing is **per proxy, not per destination**, which displaces AC54/AC63 and design-10's per-destination signing surface. `Q-10-04` is open to the **Product Manager** for the PRD amendment; it **blocks milestone M8b only**, and the design amendment follows the PRD's. **Q-10-02 RESOLVED** — 13-row at-rest inventory; payload content is at rest in exactly the three encrypted columns. Two findings: `IngestController.php:79`'s `report($e)` can write interpolated **ciphertext** into `storage/logs`, a store outside AC3's enumeration, ruled as a design change with a test rather than returned as a requirement question and flagged to the PM for awareness; and the prune-window ordering is now three-way. **D2 item 3 closed.** **ADR-008 amended, not superseded** — it had forecast the signing headers; only the credential header and the dynamic strip go beyond it. **Q-10-03 open to the Designer**, blocking nothing | `docs/plans/plan-10-sensitive-data-handling.md` (self-certified except its four Owner gates); `docs/architecture/adr-021-secret-handling-and-rotation.md`, `adr-022-inbound-verification-at-the-ingest-boundary.md`, `adr-024-field-obfuscation-and-revealed-payload-envelope.md` (**Proposed**); `adr-023-outbound-request-contract.md` (**Accepted by ratification**); `docs/design/design-10-sensitive-data-handling.md` (Approved, C1–C10 applied); `docs/product/prd-10-sensitive-data-handling.md` (Approved, 64 ACs); `docs/questions/prd-10-q-10-02-at-rest-payload-copy-inventory.md` (RESOLVED); `prd-10-q-10-03-credential-removal-and-secret-field-primitive.md`, `prd-10-q-10-04-proxy-level-signing-grain-and-live-secret-cap.md` (open) |
