...
| 10 | Sensitive data handling | **UX Design (amendment)** | **Designer** → Task Planner | **Everything upstream is approved.** PRD-10 Approved with **Amendment A and Amendment B** (Project Owner, 2026-08-27); plan-10 fully approved with **all four Owner gates approved**; ADR-021, ADR-022, ADR-024 Accepted, ADR-023 Accepted by ratification. **Amendment B re-grains outbound signing to per proxy, not per destination** — the Owner's ruling of 2026-08-27. Eight ACs revised, nothing renumbered, still 64. **AC11 changed behaviour, not only a term**: a proxy whose signing secret cannot be decrypted dispatches to **none** of its destinations, so partial fan-out is forbidden. **AC29's two-secret cap holds** — the Product Manager's ruling, approved as the PM's rather than the Owner's, with a new bullet requiring a rotation started during an overlap to state the discard before save. **AC33 untouched**: the destination credential stays per destination, excluded from the overlap. **Q-10-04 RESOLVED.** **Blocker: `design-10` needs an amendment** — its signing surface is per destination throughout (Screens 5 and 6, Flows G–I, the per-row badge), plus three stale places Q-10-04 did not name and **one genuinely new obligation**: Screen 1 / Flow B step 2 states the 24-hour promise but not that a further rotation discards the oldest. That is the one place Amendment B reaches past signing. **plan-10 needs nothing** — its R7 test (three rotations produce exactly two rows) already pins the cap. **Task Planning is unblocked for everything except the signing surface (M8b).** **Q-10-02 RESOLVED**; `IngestController.php:79` ciphertext-to-log finding ruled as a design change with a test. **D2 item 3 closed.** **Q-10-03 open to the Designer**, blocking nothing | `docs/product/prd-10-sensitive-data-handling.md` (**Approved + Amendments A and B**, 64 ACs); `docs/plans/plan-10-sensitive-data-handling.md` (**fully approved**); `docs/architecture/adr-021`, `adr-022`, `adr-023`, `adr-024` (all **Accepted**); `docs/design/design-10-sensitive-data-handling.md` (Approved — **amendment in progress**); `docs/questions/prd-10-q-10-02-…` and `prd-10-q-10-04-…` (**RESOLVED**); `prd-10-q-10-03-credential-removal-and-secret-field-primitive.md` (open) |
