...
| 10 | Sensitive data handling | **UX Design** | Designer → **Product Manager (design gate)** | **design-10 drafted, awaiting the Product Manager's approval** at the delegated design gate. No new page and no new navigation entry — it extends four existing surfaces (ProxyForm, proxy Show, a new Manage destination signing dialog, and design-06's PayloadViewer), and states explicitly that the proxies Index and events list pages are unchanged. **Four design calls are flagged for the Product Manager rather than settled**: the Verification section's placement in the form, the destination-credential Collapsible's default-expand rule, showing Credential/Signed as inline badges instead of new Destinations table columns, and whether the one-time signing-secret reveal suppresses `Esc`/overlay dismissal. **One note is folded for the Principal Engineer**: the data shape the revealed-payload endpoint returns for obfuscated JSON — the UX outcome is specified, the transport is Technical Design's call. **No question documents raised** — the PRD's UX Direction was detailed enough, including its own explicit not-the-Designer's-to-decide list. **PRD-10 APPROVED by the Project Owner on 2026-08-27, as amended** — the approval ratified `## Amendment A` whole: the V2 overturn to a closed two-scheme selector, outbound destination authentication (AC30–AC39, no longer severable), outbound request signing (AC54–AC64), and the two narrowings of approved documents (PRD-06 AC25, and a stated property of ADR-008). **Q-10-01 RESOLVED.** 64 ACs. **Q-10-02 remains open to the Principal Engineer** — the authoritative at-rest payload inventory; non-blocking, and design-10 does not depend on its answer. **ADR-008's reversal is the Principal Engineer's to record** at Technical Design | `docs/design/design-10-sensitive-data-handling.md` (**Draft**); `docs/product/prd-10-sensitive-data-handling.md` (Approved, `## Amendment A`, 64 ACs); `docs/questions/prd-10-q-10-01-outbound-destination-authentication.md` (RESOLVED); `docs/questions/prd-10-q-10-02-at-rest-payload-copy-inventory.md` (open) |
