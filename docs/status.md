...
| 10 | Sensitive data handling | **Review** | Reviewer | None blocking. **All tasks are built** — T49, the closing whole-surface manual verification pass and final regression sweep, completed 2026-08-28 (`ba0eb39`). It walked `design-10` Flows D–I against a production build in both themes at 360px and certified the delivery path on the wire, and it found and closed two stale inbound-verification references (the Screen 6 signing-dialog copy, fixed via a Designer amendment, and two class docblocks). It also recorded one open observation for a future task: a soft-deleted destination row retains its `credential_secret` ciphertext, exactly as it retains its URL and method, and no ruling requires a purge. One open doc correction sits with the Product Manager — PRD-10's Status block, see backlog — and blocks nothing | PRD-10 Approved with Amendments A, B and C (Project Owner, 2026-08-27; Amendment C by the Product Manager, 2026-08-28); `docs/plans/plan-10-sensitive-data-handling.md` fully approved, all four Owner gates; `docs/design/design-10-sensitive-data-handling.md` Approved as amended — two dated gates 2026-08-27, plus the inbound-verification withdrawal and the Screen 6 `DialogDescription` correction, both 2026-08-28; ADR-021, ADR-022, ADR-024 Accepted, ADR-023 Accepted by ratification, ADR-025 and ADR-026 Accepted (2026-08-28); tasks at `docs/tasks/sensitive-data-handling/index.md`, split per milestone, every task carrying completion notes; Q-10-02, Q-10-03, Q-10-04 and Q-10-05 all RESOLVED |
