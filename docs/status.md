...
| 4 | Queued processing (FIFO & Async) | Technical Design | — (awaiting Owner: ADR-011 + data-model) | **Blocked on Owner approval** before Task Planning: ADR-011 (Proposed) + data-model change (new `proxies.processing_mode` enum column; new `fifo_dispatches` table; new `UNIQUE(ingest_id,destination_id,attempt_number)` on `delivery_attempts`). Q-04-01 RESOLVED; mid-flight-mode-change stays within design (no UI warning, no Designer return). V3/V8 Owner-deferred, non-blocking | PRD-04 Approved (Owner 2026-08-04); design-04 Approved (PM 2026-08-04); plan-04 PE-self-certified except data-model/ADR-011 (Owner-gated) |
