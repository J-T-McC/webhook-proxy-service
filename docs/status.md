...
| 10 | Sensitive data handling | **Technical Design** | Product Manager (Q-10-04) → **Task Planner** | **plan-10 fully approved. All four Owner gates approved by the Project Owner on 2026-08-27**, so nothing in the technical design is outstanding. **Flag 1 — data model APPROVED** as enumerated: one new `proxy_secrets` table plus six columns (three on `proxies`, three on `destinations`), its only index the constraint `UNIQUE(proxy_id, purpose, is_current)`, where the nullable `is_current` is load-bearing because MySQL permits many NULLs under a unique constraint. Rollback is destructive to every stored secret. **Flags 1 and 2 were coupled and resolved the recommended way** — flag 2 approved, so flag 1's change set stands and the alternative fixed-column set is retained only as the record of what was decided against. **ADR-021, ADR-022 and ADR-024 Accepted** (Project Owner, 2026-08-27); **ADR-023 Accepted by ratification** at PRD-10's approval. **ADR-024 partially supersedes ADR-017**, adopting an alternative ADR-017 had rejected by name. **ADR-008 amended, not superseded** — it had forecast the signing headers; only the credential header and the dynamic strip go beyond it. **One blocker remains, and it is a requirements amendment rather than a gate**: the Owner's ruling of 2026-08-27 makes outbound signing **per proxy, not per destination**, which displaces AC54/AC63 and design-10's per-destination signing surface. `Q-10-04` is with the **Product Manager**; it **blocks milestone M8b only** — the signing surface. The signing backend, the destination credential, inbound verification and field obfuscation are all unblocked, so **Task Planning can start on everything except M8b**. The design amendment follows the PRD's. **Q-10-02 RESOLVED** — 13-row at-rest inventory; payload content is at rest in exactly the three encrypted columns. `IngestController.php:79`'s `report($e)` can write interpolated **ciphertext** into `storage/logs`, outside AC3's enumeration; ruled as a design change with a test and flagged to the PM for awareness. **D2 item 3 closed.** **Q-10-03 open to the Designer**, blocking nothing | `docs/plans/plan-10-sensitive-data-handling.md` (**fully approved**, 14 rulings, 10 milestones); `docs/architecture/adr-021`, `adr-022`, `adr-023`, `adr-024` (all **Accepted**); `docs/design/design-10-sensitive-data-handling.md` (Approved, C1–C10 applied); `docs/product/prd-10-sensitive-data-handling.md` (Approved, 64 ACs, **amendment pending on Q-10-04**); `docs/questions/prd-10-q-10-02-at-rest-payload-copy-inventory.md` (RESOLVED); `prd-10-q-10-03-credential-removal-and-secret-field-primitive.md`, `prd-10-q-10-04-proxy-level-signing-grain-and-live-secret-cap.md` (open) |
