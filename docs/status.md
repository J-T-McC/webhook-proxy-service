...
| 4 | Queued processing (FIFO & Async) | Implementation (rework) | Senior Developer (review-04 M-1 fix) | review-04 **Request changes**: M-1 (Major) `AdvanceProxyFifoQueue` `WithoutOverlapping` has no TTL—leaked lock after ungraceful worker crash deadlocks the sweeper's re-dispatch, defeating ADR-005(b) liveness. Fix: `expireAfter` aligned to `ingest.fifo_lease_seconds` + recovery test. Minors: createQuietly in 3 FIFO tests (fix); real-concurrency integration test (defer, no harness); T18-via-model (no action). Checks green at review. V3/V8 Owner-deferred | PRD-04/design-04/plan-04/ADR-011/tasks-04 approved; implementation T1–T25 done; review-04 Request changes (M-1 rework to SE) |
