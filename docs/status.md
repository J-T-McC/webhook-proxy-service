...
| 10 | Sensitive data handling | Backlog | — (Product Manager on start) | Not started; depends on #5. Open: **V2**. **#5's deferred concern D2 gates this PRD**; #3 left headers plaintext until this item. **Q-10-01 open (Owner → PM, 2026-08-27): does outbound destination authentication belong in this item's scope?** Outbound credentials appear nowhere on the roadmap and `destinations` has no credential column; today a secret can only reach a destination inside `destinations.url`, in plaintext. Owner leans toward merging into #10 but has not ruled. **Answer before the PRD is written — it changes the item's shape, not a detail within it** | `docs/questions/prd-10-q-10-01-outbound-destination-authentication.md` |
