...
| 4 | Queued processing (FIFO & Async) | Review (re-review) | Reviewer (review-04 M-1 verify) | M-1 **FIXED**: overlap lock now `expireAfter(fifo_lease_seconds)` + crash-recovery test (self-heals within lease, line advances); createQuietly cleanup done. Checks green (354 tests, lint/types/frontend-triad). Minors deferred/no-action per review. V3/V8 Owner-deferred | PRD-04/design-04/plan-04/ADR-011/tasks-04 approved; implementation T1–T25 done; review-04 Request changes → M-1 fixed, awaiting re-review |
