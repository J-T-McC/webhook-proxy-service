...
| Provider (shown only when the checkbox is checked) | *(did not exist — PRD-16 adds this axis of choice)* | **`Select` "Provider"**: the ten the Principal Engineer's feasibility study confirms fit the model exactly as PRD-16 specifies it — **GitHub, Stripe, Slack** (AC5's floor), **Shopify, Intercom, Zoom, PagerDuty, WooCommerce, Xero, Linear** (evidence-based additions, still Q-16-02's call, not fixed by this proposal) — a divider, **Standard Webhooks**, **Shared secret only**, a divider, **Custom**. No default selection — placeholder "Choose a provider," never silently decided (PRD-16 AC37). **Twitch, Paddle, and a Standard-Webhooks-shaped preset are deliberately not in this list** — the study found each needs an axis PRD-16 has not named yet (A1, A2, A2+A3); adding them once those axes are ruled on is a data change to this same `Select`, not a container change. | — |
| Coverage line (always visible once the checkbox is on, whether or not a Provider is chosen yet) | *(did not exist)* | Plain text under the Provider `Select`, not a tooltip: **"Works for providers that HMAC-sign a request including its body — most modern APIs. Not for signatures verified with a public key or certificate (e.g. PayPal, AWS SNS, Discord), or signatures computed over something other than the body (e.g. Twilio, Mailgun)."** See `## Coverage honesty, the tolerance clock, and the literal-timestamp trap` § Coverage honesty for why this is placed here rather than left for the member to discover by failing. | — |
| Sample request | *(did not exist)* | **`Textarea` "Sample request from your provider"**, placed directly under the Provider `Select` and coverage line (rendered as soon as the checkbox is on, before a provider is even chosen — see `## The Inbound control` § UX Direction point 1) — a single paste target for the real request's headers and body. Help: **"Paste one real request. Used to check your configuration below — never stored server-side. Have a recent one ready: the timestamp check needs a request from within your tolerance window."** Kept on-form, not a tooltip — both the retention promise and the freshness expectation are things a member needs before pasting, not after failing. See `## Coverage honesty, the tolerance clock, and the literal-timestamp trap` § The tolerance clock for the freshness design in full. | — |
