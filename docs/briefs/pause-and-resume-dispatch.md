...
- **A 4th gap, not named by Q-15-01: Async mode's original dispatch.**
  ...
  proxies, ungated by Q-15-01. AC3's "original fan-out delivery" is
  mode-agnostic, so it needs a guard too — added inside
  `ProcessIngestedWebhook::handle()`, scoped to `processing_mode !== Fifo`.
  **Not** applied to FIFO: FIFO's own claim guard already covers that path,
  and a redundant check here would (in the rare race where pause commits
  between claim and this call) make `settleOrHold` read the skipped
  dispatch as "no non-terminal deliveries" and silently settle the row as
  done — losing the event. Async has no such settle step, so it's safe there.
- **Resume, "no further member action":** FIFO dispatches
  `AdvanceProxyFifoQueue` immediately. Async has no persisted backlog (its
  deliveries are created inside `ProcessIngestedWebhook`, which paused-Async
  skipped), so resume queries this proxy's `WebhookEvent`s with no
  `deliveries` row and an uncleaned payload, and dispatches
  `ProcessIngestedWebhook` for each. Either mode: resume also dispatches
  `RetryDelivery` for the proxy's overdue `retrying` deliveries directly
  (`SweepDueRetries::forProxy()`) instead of waiting for the next tick.
...
- **The described livelock — did not match the code as literally stated.**
  Proven by direct test: claiming a Pending row for an already-cleaned event
  with zero `deliveries` rows settles and advances immediately
  (`hasNonTerminalDeliveries()` is vacuously false) — it never sticks at
  `claimed`. The real, narrower defect surfaces only once the H2 bypass
  above exists: a **replay** pre-creates `deliveries` rows before the FIFO
  claim; if the event is cleaned (now reachable mid-pause) before the
  advancer reaches that row, those rows stay non-terminal forever and the
  row never leaves `awaiting_retry`. Fixed once, at the root
  (`ProcessIngestedWebhook`'s cleaned-early-return, covering every caller —
  FIFO claim, Async direct dispatch, replay of either mode): terminalize any
  pre-existing non-terminal `deliveries` for the dispatch (CAS to `failed`,
  firing `DeliveryExhausted`, mirroring `RetryDelivery::terminalizeCleaned`'s
  shape). The zero-delivery (original-dispatch) case is an unaffected no-op.
...
Two isolated-migration tests targeted "the migration under test" via a bare
`--step=N` rollback, which selects the N most-recently-run migrations by
*position*, not by name. `add_paused_at_to_proxies_table` landing on top
made both silently roll back the wrong file (3 of 5 assertions failed when
run in isolation). Fixed by combining a wide-enough `--step` with `--path`
restricted to the named file(s) — `rollbackMigrations` skips any selected
migration outside the given path, so this is robust to future migrations.
